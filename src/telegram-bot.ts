...
class KoreanTelegramBot {
  ...
  private setupBotCommands() {
    ...
    this.bot.start((ctx) => {
      ...
      ctx.reply(
        ...
        `I'll help you learn Korean words every hour. Here's what I can do:\n\n` +
        `📚 /word - Get a random Korean word\n` +
        `🎯 /quiz - Start a quiz session\n` +
        `📖 /study - Enter study mode for multiple words\n` +
        `📊 /stats - See your learning progress\n` +
        `❓ /help - Show all commands\n\n` +
        `I'll also send you a word every hour automatically! 🕐`
      );
    });
    ...
    this.bot.help((ctx) => {
      ctx.reply(
        ...
        `💡 I'll send you a word every hour automatically!`
      );
    });
    ...
    this.bot.command('quiz', async (ctx) => {
      ...
      ctx.reply(
        `🎯 Quiz Mode Started!\n\n` +
        `I'll ask you 5 questions about Korean words. ` +
    ...
    this.bot.command('study', async (ctx) => {
      ...
      ctx.reply(
        ...
        `I'll send you Korean words with examples and explanations. ` +
        `Perfect for focused learning!\n\n` +
        `Type /word to get your first word, or wait for the hourly word! 📚`
      );
    });
  ...
  private async sendRandomWord(ctx: Context) {
    try {
      ...
      if (!word) {
        ctx.reply('No Korean words found in database. Please add some vocabulary first!');
        return;
      }
      ...
      const message = 
        `🇰🇷 **Korean Word of the Hour**\n\n` +
        `**${word.korean}** (${word.pronunciation || 'pronunciation'})\n` +
  ...
  private async sendQuizQuestion(ctx: Context) {
    try {
      ...
      const word = await this.getRandomWord();
      if (!word) {
        ctx.reply('No words available for quiz. Please add vocabulary first!');
        return;
      }
  ...
  public async start() {
    try {
      ...
      console.log('🤖 Korean Telegram Bot started successfully!');
