...
export class EnhancedKoreanBot {
...
Keep it educational, encouraging, and practical for Korean learners.`;
  ...
  private async addToPersonalVocabulary(userId: number, koreanWord: string): Promise<void> {
    try {
      // This would integrate with the existing database system
      // For now, we'll use the MCP system's user vocabulary tracking
      console.log(`Adding ${koreanWord} to personal vocabulary for user ${userId}`);
    } catch (error) {
  ...
  public stop(): void {
    this.bot.stop();
    console.log('🛑 Enhanced Korean Learning Bot stopped');
  }
}
